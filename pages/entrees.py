...
class Entrees(ft.Container):
    def __init__(self, ctpr: object):
        super(Entrees, self).__init__(
            expand=True
        )
        self.ctpr = ctpr
        ...
        self.bt_select_date = ft.IconButton(
            ...
            on_click=lambda _: self.ctpr.dp_new_entree.pick_date(),
        )
        ...
        self.ref = ft.TextField(
            **inactive_style, width=160, label="Référence"
        )
        self.des = ft.TextField(
            **inactive_style, width=350, label="Désignation",
        )
        self.unite = ft.TextField(
            **inactive_style, width=110, label="Unite",
        )
        self.num_bon = ft.TextField(
            **inactive_style, width=150, label="Numero",
        )
        self.blf = ft.TextField(
            **filter_style, width=150, label="BL",
        )
        self.id_ref = ft.Text("", visible=False)
        self.qte = ft.TextField(
            **filter_style, width=90, label="Qte", input_filter=ft.NumbersOnlyInputFilter()
        )
        self.prix = ft.TextField(
            **filter_style, width=150, label="prix", input_filter=ft.NumbersOnlyInputFilter()
        )
        self.num_bon = ft.TextField(
            **inactive_style, width=150, label="Numero",
        )
        self.comm = ft.TextField(
            **filter_style, width=600, label="Commentaire",
        )
            ...
            content=ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ft.Row(
                            controls=[
                                ft.Text("nouvelle entrée".upper(), size=16, font_family="Poppins Regular"),
        ...
        self.det_prix = ft.TextField(**filter_style, width=150, label="prix", prefix_icon="monetization_on")
        self.det_comm = ft.TextField(**filter_style, width=450, label="Commentaire")
        self.det_total = ft.TextField(**filter_style, width=150, label="Total")
                        ...
                        ft.Container(
                            ...
                            content=ft.Column(
                                controls=[
                                    ...
                                    self.det_des,
                                    ft.Row([self.det_qte, self.det_unite, self.det_prix, self.det_total]),
                                    self.det_comm,
                                    ft.Divider(height=1, color="transparent"),
                                ], spacing=20
                            )
        ...
        self.contenu = ft.Container(
            ...
            content=ft.Column(
                expand=True,
                controls=[
                    ft.Text("Entrees".upper(), size=20, font_family="Poppins Light"),
                                ...
                                ft.Row(
                                    controls=[
                                        ...
                                        ft.IconButton(
                                            ...
                                            on_click=lambda _: self.ctpr.fp_extraire_entrees_vue.save_file(
                                                allowed_extensions=['xlsx'])
                                        ),
                                        ft.IconButton(
                                            ft.icons.FILE_UPLOAD_OUTLINED, icon_size=22, icon_color="#17274f",
                                            tooltip="Exporter toutes les entrées excel",
                                            on_click=lambda _: self.ctpr.fp_extraire_entrees.save_file(
                                                allowed_extensions=['xlsx'])
                                        ),
                                        ft.IconButton(
        ...
        self.ctpr.dp_new_entree.on_change = self.change_date
        self.ctpr.fp_extraire_entrees.on_result = self.extraire_entrees
        self.ctpr.fp_extraire_entrees_vue.on_result = self.extraire_entrees_vue
        self.load_datas()
            ...
            self.table.rows.append(
                ft.DataRow(
                    ...
                    cells=[
                        ...
                        ft.DataCell(ft.Text(be.milSep(item['total']))),
    ...
    def select_ref(self, e):
        ...
        self.det_total.value = be.milSep(donnees['total'])
        self.det_comm.value = donnees['comm']
        self.det_prix.value = be.milSep(donnees['prix'])
        self.det_unite.value = be.milSep(donnees['unite'])

        for widget in (self.det_bon, self.det_bl, self.det_date, self.det_ref, self.det_des,
                       self.det_qte, self.det_prix, self.det_total, self.det_comm, self.det_unite):
            widget.update()
    ...
    def select_new_ref(self, e, f):
        ...
        self.num_bon.value = be.find_numero_bon_entree()
        self.num_bon.update()

    def creer_entree(self, e):
        count = 0
        for widget in (self.qte, self.prix, self.blf, self.sel_date):
        ...
        if count == 0:
            ...
            be.add_entree(
                self.num_bon.value, self.blf.value, self.sel_date.value,
                id_ref, qte, prix, comm
            )
            stock_actu = be.stock_by_id(id_ref)
            nouveau_stock = stock_actu + qte

            prix_actu = be.article_by_id(id_ref)[4]
            pmp = (prix_actu * stock_actu + prix * qte) / (qte + stock_actu)

            be.update_prix(pmp, id_ref)
            be.update_stock(nouveau_stock, id_ref)

            be.add_mouvement('R', self.num_bon.value, id_ref, stock_actu, nouveau_stock, qte)

            for widget in (self.blf, self.sel_date, self.num_bon, self.qte, self.prix, self.comm, self.search_ref):
                widget.value = None
                widget.update()
    ...
    def change_date(self, e):
        self.sel_date.value = str(self.ctpr.dp_new_entree.value)[0:10]
        self.sel_date.update()

    def exporter_sorties(self, e: ft.FilePickerResultEvent):
    ...
    def extraire_entrees(self, e):
        table_datas = be.all_entrees()
        ...
        prices = [row['prix'] for row in table_datas]
        totaux = [row['total'] for row in table_datas]
        ...
        entrees = {
            ...
            'prix': prices, 'total': totaux, 'commentaire': comms
        }
    ...
    def extraire_entrees_vue(self, e):
        all_datas = be.all_entrees()
